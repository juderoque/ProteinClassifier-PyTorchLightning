...
from utils.layers import MultiHeadAttention, PositionwiseFeedForward, EncoderLayer, PositionalEncoding


class LSTMAttn(pl.LightningModule):
    def __init__(self, embedding_dim, hidden_dim, seq_len, vocab_size, tagset_size,
                 padding_idx=0, d_k=64, d_v=64, n_lstm=3, n_head=12, n_attn=2, dropout=0.1, need_pos_enc=True):
...
if __name__ == '__main__':
    ...
    model = LSTMAttn(80, 200, padding, dataset.vocab_size, dataset.target_size)

    train_loader = dataset.get_train_loader(batch_size=2)
    val_loader = dataset.get_val_loader(batch_size=2)
